    ...
    public async Task<IActionResult> Remodel([FromBody] BlockCatalogueModel model)
    {
        ...
        foreach(var block in model.Blocks)
        {
            ...
            if (ct != null)
            {
                ...
                // Thumbnail URL'ini alias adıyla oluştur
                block.Thumbnail = $"/companentresim/{ct.Alias}.png";
                block.Label = ct.Name;
                block.Description = ct.Description;
                block.Icon = ct.Icon;
                
                // Ek stil bilgilerini de ekle
                block.BackgroundColor = "transparent";
                block.IconColor = "#1b264f";
            }
        }
